  ...
  \begin{Ldescription}
    \item<1->[結合$N(st)$]
      \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,semithick]
        ...
        \node[ellipse, draw, line width=0mm, fill=red!20, opacity=.7, text height =1cm, minimum width = 5cm] at (0.7,0) {$N(s)$};
        \node[ellipse, draw, line width=0mm, fill=red!20, opacity=.7, text height =1cm, minimum width = 5cm] at (3.1,0) {$N(t)$};
        \node[initial,state]     (A)                          {$q$};
    ...
    \item<2->[繰り返し$N(s*)$]
      \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,semithick]
        ...
        \path (A) edge node              {$\epsilon$} (B)
                  edge node [loop below] {$\epsilon$} (D)
              (C) edge node              {$\epsilon$} (D)
                  edge node [loop above] {$\epsilon$} (B); 
      \end{tikzpicture}
  \end{Ldescription}
\end{frame}

\subsection{決定性有限オートマトン（DFA）}
\begin{frame}[fragile]
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item<2-> 入力によって状態が一意に決まるので、状態を持つ必要がない
...
\begin{frame}[fragile]
  ...
  \uncover<2->{
    \begin{exampleblock}{}
      112775122096649474224566186997920802071913037629842902357
      700708388775797481502194024679890924858811379558008676532
      504722595317581693682693861063964492969751426762635552473
      181225392887702354610189054772960585787185954871129315742
      416465730444244942820749738732299880430019000634772118095
      75155155 
    \end{exampleblock}
    番目のGrassプログラム
  }
  ...
  \begin{itemize}
    \item<3-> バイナリにすれば122 Byte
    \item<4-> C++と比べて36 Byte差
    \item<5-> Grassの文法を表す正規表現を工夫すれば、もっと小さくなる（かも）
  \end{itemize}
\end{frame}
